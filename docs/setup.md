...
## 6.1. Контракт регистрации

Регистрация теперь работает по одному правилу:

- `cb_leads` хранит лиды и предрегистрации
- `cb_users` хранит только полноценно связанных пользователей бота
- допуск в курс завязан на наличие `email`
- `/start` не должен пускать в уроки пользователя без почты

Что считается полноценной регистрацией:

- есть запись в `cb_users`
- у записи есть `tg_id`
- у записи есть `email`

Что считается лидом:

- человек оставил почту на сайте
- или начал путь с сайта, но еще не завершил связку с ботом
- такие данные могут лежать только в `cb_leads`

Принцип работы таблиц:

- `cb_leads` не открывает курс сама по себе
- `cb_users` является единственным источником истины для прохождения курса
- `/api/telegram/auth` не должен создавать новых partial users без почты

### Сценарий 1. Пользователь пришел из бота

Поток:

1. Пользователь нажимает `/start`
2. Если в `cb_users` нет записи с `email`, бот показывает только блокирующую кнопку регистрации
3. Пользователь открывает mini app, вводит почту и завершает регистрацию
4. Route регистрации создает или обновляет запись в `cb_users`
5. `n8n` получает webhook `cb-miniapp-register` и отправляет первый урок

Ожидаемое поведение:

- до регистрации повторный `/start` снова показывает кнопку регистрации
- после регистрации урок приходит один раз в момент завершения регистрации
- следующий `/start` не должен отправлять урок заново

### Сценарий 2. Пользователь оставил на сайте только почту

Поток:

1. Сайт сохраняет данные в `cb_leads`
2. В `cb_users` записи еще нет
3. Пользователь приходит в бот и нажимает `/start`
4. Бот видит, что полноценной регистрации нет, и показывает блокирующую кнопку регистрации
5. В mini app пользователь завершает регистрацию уже внутри Telegram-контекста
6. После этого создается полноценная запись в `cb_users`, и курс стартует

Ожидаемое поведение:

- наличие одной только почты на сайте не должно открывать курс
- если на сайте не было `username`, это не проблема: он подтянется из Telegram в момент регистрации

### Сценарий 3. Пользователь оставил на сайте почту и `username`

Поток:

1. Сайт сохраняет лид в `cb_leads` c `email` и `tg_username`
2. Пользователь нажимает `/start` в боте
3. Workflow ищет совпадение по `tg_username`
4. Если совпадение найдено, создается или обновляется запись в `cb_users`
5. Лид привязывается к пользователю через `bound_user_id`
6. Если очереди уроков еще нет, бот создает очередь и отправляет первый урок

Ожидаемое поведение:

- такой пользователь считается уже готовым к входу
- отдельная блокирующая регистрация не нужна
- повторный `/start` после этого не должен слать новый урок, а только подтверждать, что пользователь уже в системе

### Сценарий 4. Повторный `/start`

Правило такое:

- если `email` нет, `/start` всегда снова показывает кнопку регистрации
- если `email` есть, но очередь уроков еще не создана, `/start` может достроить очередь и отправить текущий урок
- если `email` есть и очередь уже существует, `/start` отправляет только короткое сообщение, что пользователь уже в системе

### Почему раньше падало с `user_tg_id = undefined`

Причина была в потере контекста между нодами:

- после `Send Start Greeting` в следующую ноду мог прилетать уже не исходный `$json`
- из-за этого `Ensure Lesson Queue` иногда получал `tg_id = undefined`
- вставка в `cb_user_lessons` падала по foreign key на `cb_users`

Теперь правило такое:

- перед созданием очереди должен существовать пользователь в `cb_users`
- `Prepare Course Entry (Start)` берет данные пользователя из шага создания или upsert пользователя
- `Ensure Lesson Queue` всегда работает только с нормализованным `tg_id`
...
- блокирующий `/start`, если у пользователя еще нет почты
- автопривязку лида из `cb_leads` в `cb_users`, если на сайте уже были `email + tg_username`
