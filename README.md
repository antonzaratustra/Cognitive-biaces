...
- `cb_leads` хранит лиды и предрегистрации с сайта
- `cb_users` хранит только полноценно связанных Telegram-пользователей курса
- доступ к курсу открывается только при наличии `email`
- если пользователь пришел из бота без почты, `/start` показывает блокирующую регистрацию
- если на сайте уже есть `email + tg_username`, первый `/start` может автосвязать лид с ботом и сразу запустить курс
- повторный `/start` для уже зарегистрированного пользователя не должен повторно отправлять урок
